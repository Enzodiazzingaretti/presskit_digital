...
const OWNER  = process.env.GITHUB_OWNER  || 'enzodiazzingaretti27-design';
const REPO   = process.env.GITHUB_REPO   || 'presskit_digital';
...
const ALLOWED_FILES = ['content.json', 'dates.json'];
const IMAGE_DIR = 'img/';
...
function ghHeaders() {
  return {
    ...
    'User-Agent': 'kexxy-presskit-admin'
  };
}
...
// img/<slot>-<digits>.webp — no traversal, no arbitrary extensions.
function isAllowedImagePath(path) {
  return /^img\/[a-z0-9-]+-\d+\.webp$/.test(path);
}

module.exports = {
  OWNER, REPO, BRANCH, COOKIE_NAME, SESSION_MS, IMAGE_DIR,
  isConfigured, checkPassword,
  sign, verify, sessionCookie, currentSession, requireAuth,
